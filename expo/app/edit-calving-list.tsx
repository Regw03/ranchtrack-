...
export default function EditCalvingListScreen() {
  ...
  const list = useMemo(() => getCalvingListById(id ?? ""), [id, getCalvingListById]);

  const [name, setName] = useState<string>(list?.name ?? "");
  const [selectedColor, setSelectedColor] = useState<string>(list?.color ?? LIST_COLORS[0]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      console.log("[edit-calving-list] save error", e);

...
export default function AddAnimalsToBreedingGroupScreen() {
  const Colors = useColors();
  const { groupId } = useLocalSearchParams<{ groupId: string }>();
  const router = useRouter();
  const { animals, getBreedingGroupById, addAnimalToBreedingGroup, removeAnimalFromBreedingGroup, breedingGroups } = useRanch();
  const styles = useMemo(() => createStyles(Colors), [Colors]);
  const group = useMemo(() => getBreedingGroupById(groupId ?? ""), [getBreedingGroupById, groupId]);
  const [searchQuery, setSearchQuery] = useState("");

  const eligibleAnimals = useMemo(() => {
    const active = animals.filter((a) => (a.sex === "female" || a.sex === "heifer") && a.status === "active");
    if (!searchQuery.trim()) return active;
    const q = searchQuery.toLowerCase();
    return active.filter((a) => a.tagId.toLowerCase().includes(q) || (a.name ?? "").toLowerCase().includes(q) || a.breed.toLowerCase().includes(q));
  }, [animals, searchQuery]);

  const currentAnimalIds = useMemo(() => {
    const g = breedingGroups.find((bg) => bg.id === groupId);
    return new Set(g?.animalIds ?? []);
  }, [breedingGroups, groupId]);

  const handleToggle = useCallback((animalId: string) => {
    if (!group) return;
    if (Platform.OS !== "web") void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    if (currentAnimalIds.has(animalId)) { void removeAnimalFromBreedingGroup({ groupId: group.id, animalId }); }
    else { void addAnimalToBreedingGroup({ groupId: group.id, animalId }); }
  }, [group, currentAnimalIds, addAnimalToBreedingGroup, removeAnimalFromBreedingGroup]);

  const renderAnimalRow = useCallback(({ item }: { item: Animal }) => {
    const isInGroup = currentAnimalIds.has(item.id);
    return (
  ...
  return (
    <>
      ...
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        ...
        <View style={styles.countBar}><Text style={styles.countText}>{currentAnimalIds.size} animal{currentAnimalIds.size !== 1 ? "s" : ""} in this group</Text><Text style={styles.countHint}>Tap to add or remove</Text></View>
        <FlatList data={eligibleAnimals} keyExtractor={(item) => item.id} renderItem={renderAnimalRow} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" ListEmptyComponent={<View style={styles.emptyState}><Text style={styles.emptyText}>No matching animals found</Text></View>} />

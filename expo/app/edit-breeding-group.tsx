...
export default function EditBreedingGroupScreen() {
  const Colors = useColors();
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();
  const { getBreedingGroupById, updateBreedingGroup } = useRanch();
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await updateBreedingGroup({ ...group, name: name.trim(), color: selectedColor });
      router.back();
    } catch (e) { console.log("Error updating breeding group:", e); Alert.alert("Error", "Failed to update breeding group."); } finally { setSaving(false); }
  }, [group, name, selectedColor, saving, updateBreedingGroup, router]);
      ...
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.section}>
            ...
            <TextInput style={styles.nameInput} value={name} onChangeText={setName} placeholder="Group name" placeholderTextColor={Colors.textTertiary} maxLength={60} autoFocus />
          </View>
          <View style={styles.section}>

...
export default function CreateBreedingGroupScreen() {
  const Colors = useColors();
  const router = useRouter();
  const { createBreedingGroup } = useRanch();
  ...
  const handleSave = useCallback(async () => {
    if (!name.trim()) { Alert.alert("Name Required", "Please enter a name for your breeding group."); return; }
    if (saving) return;
    setSaving(true);
    try {
      if (Platform.OS !== "web") void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      const result = await createBreedingGroup({ name: name.trim(), color: selectedColor, animalIds: [] });
      router.back();
      setTimeout(() => { router.push({ pathname: "/breeding-group/[id]" as any, params: { id: result.newGroup.id } }); }, 100);
    } catch (e) { console.log("Error creating breeding group:", e); Alert.alert("Error", "Failed to create breeding group."); } finally { setSaving(false); }
  }, [name, selectedColor, saving, createBreedingGroup, router]);

  return (
    <>
      <Stack.Screen options={{ title: "New Breeding Group" }} />
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.previewCard}>
            ...
            <View style={styles.previewStats}>
              <Text style={styles.previewStatText}>0 animals</Text>
            </View>
          </View>
          <View style={styles.section}>
            <Text style={styles.sectionLabel}>Group Name</Text>
            <TextInput style={styles.nameInput} value={name} onChangeText={setName} placeholder='e.g. "Spring Heifers", "AI Group", "Bull Pasture"' placeholderTextColor={Colors.textTertiary} maxLength={60} autoFocus testID="breeding-group-name-input" />
          </View>
          <View style={styles.section}>
          ...
          <Text style={styles.hint}>You can add animals to this group after creating it.</Text>
        </ScrollView>
        <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) + 8 }]}>
          <TouchableOpacity style={[styles.saveBtn, !name.trim() && styles.saveBtnDisabled]} onPress={handleSave} activeOpacity={0.8} disabled={!name.trim() || saving}>

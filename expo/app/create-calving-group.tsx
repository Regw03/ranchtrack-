...
export default function CreateCalvingGroupScreen() {
  const Colors = useColors();
  const router = useRouter();
  const { createCalvingGroup } = useRanch();
  ...
  const handleSave = useCallback(async () => {
    if (!name.trim()) { Alert.alert("Name Required", "Please enter a name for your calving group."); return; }
    if (saving) return;
    setSaving(true);
    try {
      if (Platform.OS !== "web") void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      const result = await createCalvingGroup({ name: name.trim(), color: selectedColor, cowIds: [], calfIds: [] });
      router.back();
      setTimeout(() => { router.push({ pathname: "/calving-group/[id]" as any, params: { id: result.newGroup.id } }); }, 100);
    } catch (e) { console.log("Error creating calving group:", e); Alert.alert("Error", "Failed to create calving group."); } finally { setSaving(false); }
  }, [name, selectedColor, saving, createCalvingGroup, router]);

  return (
    <>
      <Stack.Screen options={{ title: "New Calving Group" }} />
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.previewCard}><View style={[styles.previewAccent, { backgroundColor: selectedColor }]} /><Text style={styles.previewName}>{name || "Group Name"}</Text><View style={styles.previewStats}><Text style={styles.previewStatText}>0 cows · 0 calves</Text></View></View>
          <View style={styles.section}><Text style={styles.sectionLabel}>Group Name</Text><TextInput style={styles.nameInput} value={name} onChangeText={setName} placeholder='e.g. "Spring Heifers 2026", "North Pasture"' placeholderTextColor={Colors.textTertiary} maxLength={60} autoFocus testID="group-name-input" /></View>
          ...
          <Text style={styles.hint}>You can add cows to this group after creating it.</Text>
        </ScrollView>
        <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) + 8 }]}><TouchableOpacity style={[styles.saveBtn, !name.trim() && styles.saveBtnDisabled]} onPress={handleSave} activeOpacity={0.8} disabled={!name.trim() || saving}><Text style={styles.saveBtnText}>{saving ? "Creating..." : "Create Group"}</Text></TouchableOpacity></View>
...
const createStyles = (Colors: ThemeColors) => StyleSheet.create({
  ...
  previewStats: { marginTop: 8 },
  previewStatText: { fontSize: 14, color: Colors.textSecondary },
  ...
  hint: { fontSize: 13, color: Colors.textTertiary, textAlign: "center" as const, fontStyle: "italic" as const },
